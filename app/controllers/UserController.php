...
class UserController extends SecureController{
	...
	function view($rec_id = null, $value = null){
		...
		$fields = array(
			...
			"email", 
			"user_role_id", 
			"password"  // Tambahkan kolom password di sini
		);
	...
	function edit($rec_id = null, $formdata = null){
		...
		// Tambahkan email ke dalam daftar field yang bisa diedit
		$fields = $this->fields = array("id_user", "username", "email", "nama", "photo", "user_role_id", "password");
	
		if ($formdata) {
			$postdata = $this->format_request_data($formdata);
	
			// Aturan validasi khusus untuk edit (email tidak required)
			$this->rules_array = array(
				'username' => 'required|regex,/^\S+$/',
				'email' => 'valid_email', // Email tidak wajib diisi
				'nama' => 'required',
				'photo' => 'required',
				'user_role_id' => 'required',
				'password' => 'min_len,6', // Password tidak wajib diisi
			);
			...
			if (!empty($modeldata['password'])) {
				$modeldata['password'] = password_hash($modeldata['password'], PASSWORD_DEFAULT);
			} else {
				unset($modeldata['password']); // Jika kosong, tidak diubah
			}
			...
			if (isset($modeldata['username'])) {
				...
				if ($db->has($tablename)) {
					$this->view->page_error[] = "Username '" . $modeldata['username'] . "' sudah digunakan!";
				}
			}
	
			// Cek apakah email sudah digunakan oleh user lain, tetapi hanya jika diisi
			if (!empty($modeldata['email'])) {
				$db->where("email", $modeldata['email'])->where("id_user", $rec_id, "!=");
				if ($db->has($tablename)) {
					$this->view->page_error[] = "Email '" . $modeldata['email'] . "' sudah digunakan!";
				}
			} else {
				unset($modeldata['email']); // Jika kosong, tidak diubah
			}
			...
			if ($this->validated()) {
				...
				if ($bool && $numRows) {
					...
					if ($db->getLastError()) {
						...
						$this->set_flash_msg("Tidak ada perubahan data", "warning");
						return $this->redirect("user");
					}
				}
			}
