...
class BaseController{
	...
	function __construct(){
		...
		$this->file_upload_settings['photo'] = array(
			"title" => "{{random}}", // Gunakan nama file acak untuk mencegah konflik nama
			"extensions" => ".jpg,.png,.gif,.jpeg", // Hanya ekstensi yang valid
			"limit" => "1", // Maksimal 1 file
			"filesize" => "3", // Batas ukuran file dalam MB
			"returnfullpath" => true, // Kembalikan path lengkap
			"filenameprefix" => "", // Tidak perlu prefix
			"uploadDir" => "uploads/files/" // Pastikan direktori ini tersedia dan writable
		);
		...
		$this->file_upload_settings['file_surat'] = array(
			"title" => "{{random}}",
			"extensions" => ".docx,.doc,.xls,.xlsx,.xml,.csv,.pdf,.xps,.jpg,.png,.jpeg,.gif", // Tambahkan ekstensi file photo
			"limit" => "1",
			"filesize" => "5",
			"returnfullpath" => true,
			"filenameprefix" => "",
		...
		if (!empty($data['file_surat'])) {
			echo '<a href="' . htmlspecialchars($data['file_surat']) . '" target="_blank">Lihat File</a>';
		}
