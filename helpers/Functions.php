...
{
    // Mendapatkan seluruh query string yang ada
    $get = filter_input_array(INPUT_GET, FILTER_SANITIZE_FULL_SPECIAL_CHARS);
    
    // Hapus parameter 'request_uri' dari query string
    unset($get['request_uri']);
    
    // Gabungkan query string yang ada dengan yang baru
    $allget = array_merge($get ?? [], $newqs);
    
    // Bangun query string jika ada
    $qs = null;
    if (!empty($allget)) {
        $qs = http_build_query($allget);
    }
    
    // Jika $pagepath kosong, gunakan URL halaman saat ini
    if (empty($pagepath)) {
        return Router::$page_url . (!empty($qs) ? "?$qs" : '');
    }
    
    // Jika $pagepath tidak kosong, gabungkan dengan query string
    return "$pagepath" . (!empty($qs) ? "?$qs" : '');
}
